...
- **Deploy**: multi-tenant via `npm run deploy:client <id>`, `npm run deploy:some <id1> <id2>`, or `npm run deploy:all`. `firebase.json` predeploy runs lint + tests; failures abort deploy.
...
- **Timezone**: For Athens-local dates use `getAthensToday()` / `formatAthensDate()` from `lib/config.js`. Store dates as UTC; convert to `Europe/Athens` only at boundaries. When storing business dates in Firestore (e.g. financial entries, payment dates) use `admin.firestore.Timestamp.fromDate(getAthensToday().utcDate)` to ensure they align with the Greek business day.
