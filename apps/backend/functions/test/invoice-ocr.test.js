...
describe('collectResponseText', () => {
  ...
  it('handles output_text property on content parts', () => {
    const response = {
      output: [{ content: [{ output_text: 'result text' }] }],
    };
  ...
  it('handles value property on content parts', () => {
    const response = {
      output: [{ content: [{ value: 'some value' }] }],
    };
  ...
  it('combines output blocks and output_text array', () => {
    const response = {
      output: [{ content: [{ text: 'from blocks' }] }],
  ...
  it('skips output blocks without content array', () => {
    const response = {
      output: [{ notContent: 'ignored' }, { content: [{ text: 'valid' }] }],
    };
    ...
    const batchSpy = vi.spyOn(visionClient, 'batchAnnotateFiles').mockResolvedValue([
      {
        responses: [
          {
            responses: [{ fullTextAnnotation: { text: 'Page 1 text from PDF' } }],
          },
        ],
      },
    ]);
    ...
    const batchSpy = vi.spyOn(visionClient, 'batchAnnotateFiles').mockResolvedValue([
      {
        responses: [
          {
            responses: [{ fullTextAnnotation: { text: 'Page 1' } }, { fullTextAnnotation: { text: 'Page 2' } }],
          },
        ],
      },
    ]);
