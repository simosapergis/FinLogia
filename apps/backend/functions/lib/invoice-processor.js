...
async function processInvoiceDocumentHandler(event) {
  ...
  try {
    ...
    if (invoiceData.isPaidAtUpload && invoicePayload.paidAmount > 0) {
      try {
        const expenseEntry = {
          type: ENTRY_TYPE.expense,
          category: EXPENSE_CATEGORY.invoicePayment,
          amount: invoicePayload.paidAmount,
          date: admin.firestore.Timestamp.fromDate(getAthensToday().utcDate),
          description: `Τιμολόγιο #${invoiceNumber || invoiceId} - ${supplierName || supplierId}`,
