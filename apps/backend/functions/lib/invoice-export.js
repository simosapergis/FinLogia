...
async function fetchInvoiceDocuments(businessId, invoicePairs) {
  ...
  for (let i = 0; i < snapshots.length; i++) {
    ...
    if (snap.exists) {
      ...
      console.warn(`Invoice not found: suppliers/${invoicePairs[i].supplierId}/invoices/${invoicePairs[i].invoiceId}`);
    }
  }
...
function sanitizeZipEntryName(invoice) {
  ...
  if (invoice.invoiceDate) {
    const date = invoice.invoiceDate.toDate ? invoice.invoiceDate.toDate() : new Date(invoice.invoiceDate);
