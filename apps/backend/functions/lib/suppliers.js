...
async function migrateSupplier({ oldSupplierId, newSupplierId, supplierUpdates }) {
  ...
  const gcsOps = invoiceDocs.map(async (invoiceDoc) => {
    ...
    const newFilePath = data.filePath.replace(`suppliers/${oldSupplierId}/`, `suppliers/${newSupplierId}/`);

    try {
      await bucket.file(data.filePath).copy(bucket.file(newFilePath));
    } catch (err) {
      console.warn(`GCS copy failed for ${data.filePath} → ${newFilePath}:`, err.message);
    }
  ...
  if (targetExists) {
    ...
    const auditOnly = Object.fromEntries(Object.entries(supplierUpdates).filter(([k]) => !PROFILE_FIELDS.includes(k)));
    mergedSupplierData = {
