...
function validatePaymentRequest(body) {
  ...
  if (body.creditInvoiceId !== undefined && typeof body.creditInvoiceId !== 'string') {
    errors.push('creditInvoiceId must be a string');
  }

  if (
    body.creditAmountUsed !== undefined &&
    (typeof body.creditAmountUsed !== 'number' || body.creditAmountUsed <= 0)
  ) {
    errors.push('creditAmountUsed must be a positive number');
  }

  if (body.creditInvoiceId && !body.creditAmountUsed) {
    errors.push('creditAmountUsed is required when creditInvoiceId is provided');
  }
...
function validateUpdateFieldsRequest(body) {
  ...
  if (fields.isCredit !== undefined && typeof fields.isCredit !== 'boolean') {
    errors.push('fields.isCredit must be a boolean');
  }
