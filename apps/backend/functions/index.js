...
export const updatePaymentStatus_v2 = onRequest(HTTP_OPTS, async (req, res) => {
  ...
  try {
    const result = await db.runTransaction(async (tx) => {
      // 3. Fetch documents
      const docsToGet = [invoiceRef];
      if (creditInvoiceRef) docsToGet.push(creditInvoiceRef);

      const snaps = await tx.getAll(...docsToGet);
      const invoiceSnap = snaps[0];
      const creditInvoiceSnap = snaps[1];
      ...
      // 5. Authorization check - only owner can update
      ...
      // 6. Handle credit invoice if provided
      let creditData = null;
      let actualCreditUsed = 0;
      if (creditInvoiceRef) {
        if (!creditInvoiceSnap.exists) {
          throw Object.assign(new Error(`Credit invoice not found: ${creditInvoiceId}`), { httpStatus: 404 });
        }
        creditData = creditInvoiceSnap.data();
        if (!creditData.isCredit) {
          throw Object.assign(new Error(`Invoice ${creditInvoiceId} is not a credit invoice`), { httpStatus: 400 });
        }
        const creditUnpaid = creditData.unpaidAmount || 0;
        if (creditUnpaid < creditAmountUsed) {
          throw Object.assign(
            new Error(`Credit amount used (${creditAmountUsed}) exceeds available credit (${creditUnpaid})`),
            { httpStatus: 400 }
          );
        }
        actualCreditUsed = creditAmountUsed;
      }

      // 7. Get current payment state
      ...
      // 9. Validate payment doesn't exceed remaining balance
      ...
      // 10. Validate not already fully paid
      ...
      // 11. Calculate actual cash amount
      const cashAmount = Math.max(0, paymentAmount - actualCreditUsed);

      // 12. Calculate new amounts for target invoice
      ...
      // 13. Build payment history entry
      let finalNotes = notes || '';
      if (actualCreditUsed > 0) {
        const creditNote = `Έγινε χρήση του πιστωτικού με αριθμό ${creditData.invoiceNumber || creditInvoiceId}`;
      ...
      const paymentEntry = {
        ...
        creditInvoiceId: creditInvoiceId || null,
        creditAmountUsed: actualCreditUsed > 0 ? actualCreditUsed : null,
      ...
      if (creditInvoiceRef && actualCreditUsed > 0) {
        const creditTotalAmount = creditData.totalAmount || 0;
        const creditCurrentPaid = creditData.paidAmount || 0;
        const creditNewPaid = creditCurrentPaid + actualCreditUsed;
        const creditNewUnpaid = Math.max(0, creditTotalAmount - creditNewPaid);
        const creditNewStatus = derivePaymentStatus(creditNewPaid, creditTotalAmount);

        tx.update(creditInvoiceRef, {
          paymentStatus: creditNewStatus,
          paidAmount: creditNewPaid,
          unpaidAmount: creditNewUnpaid,
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
      }
    ...
    // Auto-create expense entry for the payment ONLY for cashAmount
...
export const updateInvoiceFields_v2 = onRequest(HTTP_OPTS, async (req, res) => {
  ...
  try {
    const result = await db.runTransaction(async (tx) => {
      ...
      if (fields.isCredit !== undefined) {
        updates.isCredit = fields.isCredit;
        updatedFields.push('isCredit');
      }
...
export const updateSupplierFields_v2 = onRequest(HTTP_OPTS, async (req, res) => {
  ...
  const newSupplierId =
    fields.supplierTaxNumber !== undefined ? sanitizeId(fields.supplierTaxNumber, supplierId) : null;
...
export const editFinancialEntry_v2 = onRequest(HTTP_OPTS, async (req, res) => {
  ...
  try {
    ...
    if (fields.type !== undefined && fields.type !== current.type && fields.category === undefined) {
      const validCategories = effectiveType === ENTRY_TYPE.income ? VALID_INCOME_CATEGORIES : VALID_EXPENSE_CATEGORIES;
