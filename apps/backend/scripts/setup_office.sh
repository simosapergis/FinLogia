...
handle_error() {
    ...
    echo "3. Ensure you have the necessary IAM permissions on the project, including roles/logging.configWriter and roles/cloudconfig.admin."
    exit $exit_code
}
...
# ── 12. Usage Telemetry Remote Config ──────────────────────────────────────────

if is_step_done "usage_remote_config"; then
    echo -e "[11/${TOTAL_STEPS}] Usage telemetry Remote Config already configured. Skipping..."
else
    echo -e "[11/${TOTAL_STEPS}] Configuring usage telemetry Remote Config switch..."
    ./scripts/setup_remote_config.sh "$PROJECT_ID" --telemetry-enabled true >> "$LOG_FILE" 2>&1
    echo -e "  -> telemetry_enabled defaults to true and can be changed without deployment."
    mark_step_done "usage_remote_config"
fi

# ── 13. Environment Configuration ──────────────────────────────────────────────

if is_step_done "env_config"; then
    echo -e "[12/${TOTAL_STEPS}] Environment configuration already generated. Skipping..."
else
    echo -e "[12/${TOTAL_STEPS}] Generating .env configuration..."
    if [ -z "$SA_EMAIL" ]; then
        SA_EMAIL=$(gcloud iam service-accounts list --project "$PROJECT_ID" --format="value(email)" --filter="email:firebase-adminsdk")
    fi
...
if is_step_done "auth_setup"; then
    echo -e "[13/${TOTAL_STEPS}] Authentication already configured. Skipping..."
else
    echo -e "[13/${TOTAL_STEPS}] Configuring Email/Password authentication..."
...
# ── 15. Artifact Registry Cleanup Policy ──────────────────────────────────────

if is_step_done "artifact_cleanup"; then
    echo -e "[14/${TOTAL_STEPS}] Artifact Registry cleanup policy already set. Skipping..."
else
    echo -e "[14/${TOTAL_STEPS}] Configuring Artifact Registry cleanup policy..."
...
if is_step_done "deployment"; then
    echo -e "[15/${TOTAL_STEPS}] Deployment already completed. Skipping..."
else
    echo -e "[15/${TOTAL_STEPS}] Deploying Cloud Functions (this may take a few minutes)..."
...
if is_step_done "register_web_app"; then
    echo -e "[16/${TOTAL_STEPS}] Firebase Web App already registered. Skipping..."
else
    echo -e "[16/${TOTAL_STEPS}] Registering Firebase Web App & exporting config..."
