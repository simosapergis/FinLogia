          ...
          <div class="sm:flex sm:items-start">
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
              <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                Προσθήκη Χρήστη σε Επιχείρηση
              </h3>
              <div class="mt-4">
                <form @submit.prevent="submitForm" class="space-y-4">
                  <div>
                    <label for="businessId" class="block text-sm font-medium text-gray-700">Επιχείρηση</label>
                    <select id="businessId" v-model="form.businessId" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                      <option disabled value="">Επιλέξτε επιχείρηση</option>
                  ...
                  <div>
                    <label for="displayName" class="block text-sm font-medium text-gray-700">Ονοματεπώνυμο</label>
                    <input type="text" id="displayName" v-model="form.displayName" required autocomplete="name" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                  </div>
                  <div>
                    <label for="password" class="block text-sm font-medium text-gray-700">Κωδικός (προαιρετικό)</label>
                  ...
                  <div>
                    <label for="role" class="block text-sm font-medium text-gray-700">Ρόλος</label>
                    <select id="role" v-model="form.role" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                      <option value="owner">Ιδιοκτήτης (Owner)</option>
                      <option value="employee">Υπάλληλος (Employee)</option>
                      <option value="viewer">Αναγνώστης (Viewer)</option>
                    </select>
                  </div>
                </form>
              </div>
...
const submitForm = async () => {
  if (!form.value.businessId || !form.value.email || !form.value.displayName || !form.value.role) return;
  isSubmitting.value = true;
  try {
    await emit('submit', { ...form.value });
    form.value = { businessId: '', email: '', displayName: '', password: '', role: 'employee' };
  } finally {
