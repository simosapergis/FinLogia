...
import { viewInvoiceDetails, getSignedInvoiceUrl } from '@/services/api/invoicesApi';
...
watch(
  ...
  async (isVisible) => {
    ...
    try {
      const detailsRes = await viewInvoiceDetails({
        clientProjectId: props.clientProjectId,
        supplierId: props.supplierId,
        invoiceId: props.invoiceId,
      });
      invoice.value = detailsRes.data.invoice;

      if (invoice.value?.filePath) {
        try {
          const urlRes = await getSignedInvoiceUrl({
            clientProjectId: props.clientProjectId,
            filePath: invoice.value.filePath,
            bucketName: props.bucketName,
          });
          pdfUrl.value = urlRes.data.downloadUrl;
        } catch {
